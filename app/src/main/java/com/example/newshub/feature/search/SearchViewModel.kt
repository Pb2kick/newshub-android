...
class SearchViewModel(
    ...
    fun search(query: String, location: String, scope: String) {
        ...
        viewModelScope.launch {
            val articles = when (val result = backendService.searchNews(trimmed, location, scope)) {
                is ApiResult.Success -> result.data
                is ApiResult.Failure -> emptyList()
            }
            val elections = when (val result = backendService.searchElections(trimmed)) {
                is ApiResult.Success -> result.data
                is ApiResult.Failure -> emptyList()
            }
            val candidates = when (val result = backendService.searchCandidates(trimmed)) {
                is ApiResult.Success -> result.data
                is ApiResult.Failure -> emptyList()
            }
